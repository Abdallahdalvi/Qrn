...
export class RecitationTracker {
  private utteranceAudio: any = new Float32Array(0);
  ...
  async feed(samples: Float32Array): Promise<WorkerOutbound[]> {
    ...
    this.utteranceAudio = concatFloat32(this.utteranceAudio, samples) as unknown as Float32Array;
    const maxSamples =
  ...
  private async _handleTracking(finalFlush: boolean): Promise<WorkerOutbound[]> {
    ...
    messages.push({
      ...
      confidence: Math.min(1.0, 0.70 + (cumulativeCoverage * 0.30)), // Rough confidence based on progress
    });
